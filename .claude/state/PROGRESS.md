# Progress Log
[2026-06-11 16:00] tests (#183): Vitest harness landed — 77 tests / 5 suites, 250ms, no live Supabase (chainable mock client in tests/helpers/). Auth gates table-driven across all 15 admin route/method pairs; bulk caps; decide-* helpers; reviews source filter; activity union. Mutation-tested (401->200 swap caught). npm test wired into CI before build. Why: wide-viewport bug shipped on manual smoke only.
