...
export function buildAssistSystemPrompt(): string {
  ...
  return [
    ...
    '',
    'Evidence rules (critical — false negatives are worse than false positives):',
    '- ABSENCE of study signals is NOT evidence of unsuitability. Google returns at',
    '  most 5 sampled reviews and a few photos; a cafe whose sample never mentions',
    '  laptops is UNKNOWN, not unsuitable. Suggest "rejected" only on POSITIVE',
    '  evidence of a rejection reason (a no-laptop sign, takeout-only layout, not a',
    '  cafe at all, clearly hostile to working).',
    '- Weight the review summaries most heavily: they aggregate the FULL review',
    '  corpus, while the individual reviews are a small sample.',
    '- When evidence is thin or mixed, suggest "deferred" with low confidence and',
