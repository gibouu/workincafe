...
- **Remove `passWithNoTests` from `vitest.config.ts`** when the first Tier 1
  test is introduced, and no later than the first product vertical-slice PR.
  The foundation ships it because no implementation test exists yet; the test
  command must not indefinitely succeed without discovering any tests.
